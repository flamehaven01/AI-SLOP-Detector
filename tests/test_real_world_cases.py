...
def test_case_1_ai_slop(detector):
    ...
    """
    test_file = Path("D:/Sanctum/ai-slop-detector/tests/corpus/test_case_1_ai_slop.py")
    
    result = detector.analyze_file(str(test_file))
    
    # Should detect as slop
    assert result.status in [SlopStatus.CRITICAL_DEFICIT, SlopStatus.SUSPICIOUS, SlopStatus.INFLATED_SIGNAL]
    
    # High inflation due to buzzwords
    assert result.inflation.inflation_score > 0.5, \
        f"Inflation too low: {result.inflation.inflation_score:.2f}"
    
    # Low LDR due to empty functions
    assert result.ldr.ldr_score < 0.5, \
        f"LDR too high: {result.ldr.ldr_score:.2f}"
    
    # Should detect multiple pattern issues
    assert len(result.pattern_issues) >= 3, \
    ...
    assert result.deficit_score > 30, \
        f"Deficit score too low: {result.deficit_score:.2f}"
...
def test_case_2_fake_docs(detector):
    ...
    """
    test_file = Path("D:/Sanctum/ai-slop-detector/tests/corpus/test_case_2_fake_docs.py")
    
    result = detector.analyze_file(str(test_file))
    
    # Should detect inflated signal
    assert result.status in [SlopStatus.INFLATED_SIGNAL, SlopStatus.SUSPICIOUS, SlopStatus.CRITICAL_DEFICIT]
    
    # Very high inflation - lots of buzzwords in docstrings
    assert result.inflation.inflation_score > 1.0, \
        f"Inflation should be > 1.0, got {result.inflation.inflation_score:.2f}"
    
    # Many jargon terms
    assert result.inflation.jargon_count > 10, \
...
def test_case_3_hyped_comments(detector):
    ...
    - Bare except pattern
    """
    test_file = Path("D:/Sanctum/ai-slop-detector/tests/corpus/test_case_3_hyped_comments.py")
    
    result = detector.analyze_file(str(test_file))
    
    # Should detect as problematic
    assert result.status in [SlopStatus.SUSPICIOUS, SlopStatus.INFLATED_SIGNAL, SlopStatus.CRITICAL_DEFICIT]
    ...
    pattern_ids = [issue.pattern_id for issue in result.pattern_issues]
    comment_patterns = [pid for pid in pattern_ids if any(x in pid for x in ["todo", "fixme", "xxx", "hack"])]
    assert len(comment_patterns) >= 2, \
...
def test_generate_markdown_report(detector, tmp_path):
    ...
    """
    test_files = [
        ("Test Case 1: AI Slop", "D:/Sanctum/ai-slop-detector/tests/corpus/test_case_1_ai_slop.py"),
        ("Test Case 2: Fake Docs", "D:/Sanctum/ai-slop-detector/tests/corpus/test_case_2_fake_docs.py"),
        ("Test Case 3: Hyped Comments", "D:/Sanctum/ai-slop-detector/tests/corpus/test_case_3_hyped_comments.py"),
    ]
    
    report_lines = [
        "# AI-SLOP Detector - Test Results Report",
        "",
        f"**Generated:** 2026-01-09",
        "",
        "---",
        "",
    ]
    ...
    corpus_report = Path("D:/Sanctum/ai-slop-detector/tests/corpus/DETECTION_REPORT.md")
