...
class TrainingDataCollector:
    ...
    def collect_good_data(self, max_files: int = 1000) -> List[Path]:
        ...
        for owner, repo in self.GOOD_REPOS:
            ...
            filtered_files = [
                ...
                if not any(part in f.parts for part in ["test", "tests", "docs", "doc", "examples", "example"])
            ]
    ...
    def extract_features(self, file_path: Path, detector) -> Optional[TrainingExample]:
        ...
        try:
            ...
            try:
                ...
                if functions:
                    total_func_lines = sum(node.end_lineno - node.lineno for node in functions if hasattr(node, 'end_lineno'))
            ...
            cross_language_count = sum(1 for issue in result.pattern_issues if "cross-language" in issue.category.lower())
            hallucination_count = sum(1 for issue in result.pattern_issues if "hallucination" in issue.category.lower())
    ...
    def build_dataset(self, detector, good_limit: int = 1000, bad_limit: int = 500) -> Tuple[List[TrainingExample], List[TrainingExample]]:
    ...
    def save_dataset(self, good_examples: List[TrainingExample], bad_examples: List[TrainingExample], output_path: Path):
        ...
        dataset = {
            ...
            "metadata": {
                ...
                "feature_names": [
                    ...
                    "total_lines", "logic_lines", "empty_lines", "avg_complexity"
                ]
            }
        }
